...
def generate_distances_network(workers):
    ...
    os.system("rm -f "+returnPathStruc2vec()+"/pickles/weights_distances-layer-*.pickle")
    ...
    t0 = time()
    os.system("rm -f "+returnPathStruc2vec()+"/pickles/graphs-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
    ...
    t0 = time()
    os.system("rm -f "+returnPathStruc2vec()+"/pickles/distances_nets_weights-layer-*.pickle")
    os.system("rm -f "+returnPathStruc2vec()+"/pickles/alias_method_j-layer-*.pickle")
    os.system("rm -f "+returnPathStruc2vec()+"/pickles/alias_method_q-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
