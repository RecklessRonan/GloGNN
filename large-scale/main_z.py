...
from data_utils import evaluate_acmgcn, normalize, gen_normalized_adjs, evaluate, evaluate_mlpnorm, evaluate_mlpnormz, evaluate_acmgcn, eval_acc, eval_rocauc, to_sparse_tensor, load_fixed_splits
...
if args.method == 'mlpnorm' or args.method == 'mlpnorm_z':
...
case_study_dir = 'case_study/' + str(args.dataset)
edge_indx_dir = case_study_dir + '/edge_index.pt'
torch.save(dataset.graph['edge_index'], edge_indx_dir)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if not args.sampling:
            optimizer.zero_grad()
            if args.method == 'mlpnorm' or args.method == 'ggcn' or args.method == 'mlpnorm_z':
                # print('x', x.device)
                # print(x.shape)
                ...
                out, z = model(x, adj)
                case_study_dir = 'case_study/' + str(args.dataset)
                torch.save(case_study_dir + '/z_'+str(epoch)+'.pt')
            elif args.method == 'acmgcn':
        ...
        elif args.method == 'mlpnormz':
            result = evaluate_mlpnormz(model, x, adj, dataset, split_idx, eval_func,
                                       sampling=args.sampling, subgraph_loader=subgraph_loader)
        elif args.method == 'acmgcn':
